...
CREATE TABLE nd_tchr (
  mno    INTEGER NOT NULL COMMENT '강사 번호', -- 강사 번호
...
-- 신청자
ALTER TABLE nd_stud
  ADD CONSTRAINT PK_nd_stud -- 신청자 기본키
    PRIMARY KEY (
      mno -- 수강생번호
    );
...
CREATE TABLE nd_lesn (
  ...
  mno    INTEGER      NULL     COMMENT '매니저 번호' -- 매니저 번호
)
COMMENT '수업';
...
ALTER TABLE nd_cls_room
  ADD CONSTRAINT PK_nd_cls_room -- 교육기관 기본키
    PRIMARY KEY (
      cno -- 강의실번호
    );
...
ALTER TABLE nd_lesn_tchr
  ADD CONSTRAINT PK_nd_lesn_tchr -- 강사 배정 기본키
    PRIMARY KEY (
      lno, -- 수업번호
      mno  -- 강사 번호
    );
...
-- 수강신청
ALTER TABLE nd_appl
  ADD CONSTRAINT FK_nd_stud_TO_nd_appl -- 신청자 -> 수강신청
    FOREIGN KEY (
      mno -- 수강생번호
    )
    REFERENCES nd_stud ( -- 신청자
      mno -- 수강생번호
    );
...
-- 신청자
ALTER TABLE nd_stud
  ADD CONSTRAINT FK_nd_addr_TO_nd_stud -- 주소 -> 신청자
    FOREIGN KEY (
      ano -- 주소번호
    )
...
-- 신청자
ALTER TABLE nd_stud
  ADD CONSTRAINT FK_nd_memb_TO_nd_stud -- 회원 -> 신청자
    FOREIGN KEY (
      mno -- 수강생번호
    )
    REFERENCES nd_memb ( -- 회원
...
-- 수업
ALTER TABLE nd_lesn
  ADD CONSTRAINT FK_nd_cls_room_TO_nd_lesn -- 교육기관 -> 수업
    FOREIGN KEY (
      cno -- 강의실번호
    )
    REFERENCES nd_cls_room ( -- 교육기관
      cno -- 강의실번호
    );
...
-- 강사 배정
ALTER TABLE nd_lesn_tchr
  ADD CONSTRAINT FK_nd_tchr_TO_nd_lesn_tchr -- 강사 -> 강사 배정
    FOREIGN KEY (
      mno -- 강사 번호
    )
...
-- 강사 배정
ALTER TABLE nd_lesn_tchr
  ADD CONSTRAINT FK_nd_lesn_TO_nd_lesn_tchr -- 수업 -> 강사 배정
    FOREIGN KEY (
      lno -- 수업번호
    )
    REFERENCES nd_lesn ( -- 수업
